...
class KPDataset(Dataset):
    def __init__(self, data_list: List, input_shape: Tuple[int]=(416, 416), sigma: int=3):
        """
    ...
    def __getitem__(self, idx):
        ...
        img_tensor = torch.tensor(img).permute(2, 0, 1).float().to("cuda")
        hm_tensor = torch.tensor(hm).unsqueeze(0).float().to("cuda")
...
def build_train_loader(cfg):
    ...
    dataset = KPDataset(data_list, cfg.MODEL.INPUT_SHAPE, cfg.DATASETS.SIGMA)
